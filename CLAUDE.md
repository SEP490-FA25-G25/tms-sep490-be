...
public StudentClassDTO getStudentClass(Long studentId, Long classId) {
...
public interface StudentClassQueryStrategy {
    StudentClassDTO execute(StudentClassQuery query);
}

@Component
public class CachedStudentClassQueryStrategy implements StudentClassQueryStrategy {
    private final CacheManager cacheManager;
    private final EventPublisher eventPublisher;
    private final MetricsCollector metricsCollector;

    @Cacheable(value = "studentClasses", key = "#query.compositeKey")
    @Measured(value = "student.class.query.duration")
    public StudentClassDTO execute(StudentClassQuery query) {
        // ... 50+ lines of abstraction for a simple query
    }
}
```
