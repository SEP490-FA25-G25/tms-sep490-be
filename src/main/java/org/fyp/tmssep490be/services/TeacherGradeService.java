...
=======
    ...
    public TeacherStudentScoreDTO saveOrUpdateScore(Long teacherId, Long assessmentId, ScoreInputDTO scoreInput) {
        ...
        boolean hasScore = scoreInput.getScore() != null;

        // Cho phép lưu nhận xét mà không cần nhập điểm
        score.setScore(hasScore ? java.math.BigDecimal.valueOf(scoreInput.getScore()) : null);
        score.setFeedback(scoreInput.getFeedback());

        if (hasScore) {
            // Chỉ set thông tin chấm điểm khi thực sự có điểm
            score.setGradedBy(teacherRepository.findById(teacherId).orElse(null));
            score.setGradedAt(now);
        } else {
            // Không có điểm: giữ trạng thái chưa chấm
...
>>>>>>> 77a2493 (feat: refine reschedule, notifications and grading backend)
}
