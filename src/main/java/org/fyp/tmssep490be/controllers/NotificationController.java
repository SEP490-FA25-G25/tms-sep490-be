...
public class NotificationController {
    ...
    public ResponseEntity<ResponseObject<Page<NotificationDTO>>> getNotifications(
        ...
        try {
            ...
            if (priority != null) {
                try {
                    filter.setPriority(org.fyp.tmssep490be.entities.enums.NotificationPriority.valueOf(priority.toUpperCase()));
                } catch (IllegalArgumentException e) {
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER', 'CENTER_HEAD', 'ACADEMIC_AFFAIR')")
    public ResponseEntity<ResponseObject<Page<NotificationDTO>>> getNotificationsByReference(
        ...
        try {
            Pageable pageable = PageRequest.of(page, size, Sort.by(Sort.Direction.DESC, "createdAt"));
            Page<NotificationDTO> notifications = notificationService.getNotificationsByReference(
                    referenceType, referenceId, pageable);

            return ResponseEntity.ok(ResponseObject.<Page<NotificationDTO>>builder()
                    .success(true)
                    .message("Lấy thông báo theo reference thành công")
                    .data(notifications)
                    .build());

        } catch (Exception e) {
            log.error("Lỗi khi lấy thông báo theo reference", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ResponseObject.<Page<NotificationDTO>>builder()
                            .success(false)
                            .message("Lỗi khi lấy thông báo theo reference: " + e.getMessage())
                            .build());
        }
    }
}