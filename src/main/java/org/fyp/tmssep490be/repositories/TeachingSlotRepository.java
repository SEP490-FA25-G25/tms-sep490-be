...
public interface TeachingSlotRepository extends JpaRepository<TeachingSlot, TeachingSlot.TeachingSlotId> {
    ...
    // Kiểm tra có teaching slot gán cho session không
    ...
    // Lấy teaching slots theo session id và status
    @Query("SELECT ts FROM TeachingSlot ts JOIN FETCH ts.teacher t JOIN FETCH t.userAccount WHERE ts.session.id = :sessionId AND ts.status = :status")
    List<TeachingSlot> findBySessionIdAndStatus(
