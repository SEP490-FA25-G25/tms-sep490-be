...
public interface SessionResourceRepository extends JpaRepository<SessionResource, SessionResource.SessionResourceId> {
   ...
   * query instead of N)
   * <p>
   * Now checks for TIME OVERLAP instead of just matching time slot IDs.
   * Example: Slot1 (7:00-8:30) and Slot2 (8:00-10:30) are detected as
   * overlapping.
   * </p>
   *
  ...
  @Query(value = """
      WITH current_class_schedule AS (
        -- Get all unique (date, time_slot start/end) pairs from current class
        SELECT DISTINCT s.date, s.time_slot_template_id, tst.start_time, tst.end_time
        FROM session s
        JOIN time_slot_template tst ON s.time_slot_template_id = tst.id
      ...
      resource_bookings AS (
        ...
        SELECT DISTINCT
          sr.resource_id,
          s.date,
          tst.start_time as booking_start,
          tst.end_time as booking_end
      ...
      INNER JOIN current_class_schedule ccs
        ON rb.date = ccs.date
        -- Time overlap check: booking overlaps with current class session
        AND rb.booking_start < ccs.end_time
        AND rb.booking_end > ccs.start_time
  ...
  // Tìm tất cả session resources theo resource và ngày (để kiểm tra overlap
  // conflict)
  @Query("SELECT sr FROM SessionResource sr " +
      "JOIN FETCH sr.session s " +
      "JOIN FETCH s.timeSlotTemplate " +
  ...
  /**
   * Delete all resource assignments for sessions on a specific day of week
   * This should be called BEFORE bulkInsertResourcesForDayOfWeek to replace
   * resources
   *
  ...
  @Query(value = """
      ...
      """, nativeQuery = true)
  int deleteResourcesForDayOfWeek(
   ...
   * - Have no TIME OVERLAP conflict with other classes using this resource
   * 
   * NOTE: Now checks for actual time overlap instead of just matching time slot
   * IDs.
   * NOTE: Call deleteResourcesForDayOfWeek BEFORE this method to replace existing
   * resources!
   *
   * @param classId    class ID
   * @param dayOfWeek  day of week (PostgreSQL DOW: 0=Sunday, 1=Monday, ...,
  ...
  @Query(value = """
      ...
      FROM session s
      JOIN time_slot_template tst1 ON s.time_slot_template_id = tst1.id
      WHERE s.class_id = :classId
        AND EXTRACT(DOW FROM s.date) = :dayOfWeek
        AND NOT EXISTS (
          SELECT 1 FROM session_resource sr2
          JOIN session s2 ON sr2.session_id = s2.id
          JOIN time_slot_template tst2 ON s2.time_slot_template_id = tst2.id
          WHERE sr2.resource_id = :resourceId
            AND s2.date = s.date
            -- Time overlap check instead of ID match
            AND tst2.start_time < tst1.end_time
            AND tst2.end_time > tst1.start_time
            AND s2.id != s.id
        )
      """, nativeQuery = true)
   ...
   * assigned
   * in Phase 1 (SQL Bulk) due to TIME OVERLAP conflicts.
   *
   * @param classId    class ID
   * @param dayOfWeek  day of week (PostgreSQL DOW: 0=Sunday, 1=Monday, ...,
   *                   6=Saturday)
  ...
  @Query(value = """
      ...
      LEFT JOIN session s2 ON sr2.session_id = s2.id
                          AND s2.date = s.date
                          AND s2.id != s.id
      LEFT JOIN time_slot_template tst2 ON s2.time_slot_template_id = tst2.id
      WHERE s.class_id = :classId
        AND EXTRACT(DOW FROM s.date) = :dayOfWeek
   ...
   * Now checks for TIME OVERLAP instead of matching time slot IDs.
   */
  @Query(value = """
      SELECT s2.id as conflicting_session_id,
             ...
             tst2.start_time as conflicting_time_start,
             tst2.end_time as conflicting_time_end
      FROM session s1
      JOIN time_slot_template tst1 ON s1.time_slot_template_id = tst1.id
  ...
  /**
   * Delete all session resources for a class (wizard reset)
   */
  @org.springframework.data.jpa.repository.Modifying
  @Query(value = """
      DELETE FROM session_resource sr
      WHERE sr.session_id IN (
          SELECT s.id FROM session s WHERE s.class_id = :classId
      )
      """, nativeQuery = true)
  int deleteAllByClassId(@Param("classId") Long classId);
}